...
function drift(t2, phase, amp, p1 = 8.3, p2 = 13.7) {
  return amp * (0.62 * Math.sin(2 * Math.PI * t2 / p1 + phase) + 0.38 * Math.sin(2 * Math.PI * t2 / p2 + phase * 1.7));
}
var REST = {
  opacity: 1,
  x: 0,
...
function cameraTransform(c, t2 = 0, w = 430, h = 932) {
  const bx = drift(t2, 0, 2, 11.3, 17.9);
  const by = drift(t2, 1.9, 3.4, 9.7, 15.1);
  const bs = 1 + drift(t2, 3.4, 45e-4, 12.9, 20.3);
...
function evaluate(tracks, t2) {
  ...
  let camera = { ...CAMERA_REST };
  {
    ...
    for (const tr of tracks.filter((x) => x.k === "cam").sort((a, b) => a.at - b.at)) {
      ...
      if (t2 >= tr.at) camera = focusPush(progress(t2, tr.at, tr.dur), from, to);
      from = to;
    }
  }
  let focus = { ...FOCUS_REST };
  {
    ...
    for (const tr of tracks.filter((x) => x.k === "focus").sort((a, b) => a.at - b.at)) {
      const to = { ...from, ...tr.to };
      if (t2 >= tr.at) focus = lerpFocus(from, to, progress(t2, tr.at, tr.dur));
      from = to;
    }
  }
  for (const tr of [...tracks].sort((a, b) => a.at - b.at)) {
    if (tr.k === "stagger") {
      ...
      tr.ids.forEach((id, i) => {
        const q = ease(progress(t2, tr.at + i * step, tr.dur));
  ...
  return { layers, els, clips, camera, focus, t: t2 };
}
var bandCache = [];
function build(root, format = "vertical") {
...
function paint(frame) {
  ...
  cam.style.transform = cameraTransform(frame.camera, t2);
  for (const [id, s] of Object.entries(frame.layers)) {
    ...
    const dx = s.x + drift(t2, id.length * 0.9, 1.3, 10.1, 16.3);
    const dy = s.y + drift(t2, id.length * 1.4 + 0.6, 2.9, 8.9, 14.7);
  ...
  for (const b of bandCache) {
    const bx = drift(t2, b.phase, 1.1 * b.depth, 9.3, 15.9);
    const by = drift(t2, b.phase + 0.8, 2.6 * b.depth, 7.7, 12.1);
...
var audio = new Audio("../audio/mix.wav");
var section = SECTION.full;
var playing = false;
var t = 0;
var started = 0;
...
var bar = document.querySelector(".sec");
for (const [name, s] of Object.entries(SECTION)) {
  ...
  b.addEventListener("click", () => {
    stop();
    section = s;
    scrub.min = String(s.from);
    scrub.max = String(s.to);
    show(s.from);
  });
  bar.append(b);
}
